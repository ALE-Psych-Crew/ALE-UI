...
class ALEColorPicker extends ALEUISpriteGroup
{
	...
	public function new(?x:Float, ?y:Float, ?color:FlxColor, ?w:Float, ?h:Float)
	{
		...
		var intW:Float = Math.floor(w ?? ALEUIUtils.OBJECT_SIZE * 6);
		var intH:Float = Math.floor(h ?? ALEUIUtils.OBJECT_SIZE * 6);

		spriteHSB = new ALEMouseSprite().makeGraphic(intW, intH, FlxColor.BLACK);
		add(spriteHSB);
		...
		selHSB = new ALEUISprite().makeGraphic(radius, radius, FlxColor.BLACK);
		ALEUIUtils.outlineBitmap(selHSB.pixels);
		add(selHSB);
		selHSB.offset.x = selHSB.offset.y = radius / 2;
		selHSB.angle = 45;

		spriteHUE = new ALEMouseSprite(0, intH * 1.1).makeGraphic(intW, Math.floor(intH / 7.5), FlxColor.BLACK);
		...
		selHUE = new ALEUISprite().makeGraphic(radius * 0.6, Math.floor(intH / 6), FlxColor.BLACK);
